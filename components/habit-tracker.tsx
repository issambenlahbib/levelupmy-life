 ...
 {!loading && (
  <div className="w-full overflow-x-auto">
    <div
      className="grid"
      style={{
        gridTemplateColumns: `200px repeat(${currentDays.length}, 1fr)`,
        minWidth: `${200 + currentDays.length * 100}px`,
      }}
    >
      {/* Top-left corner */}
      <div className="font-bold text-electric-purple bg-white border border-electric-blue/20 sticky left-0 z-10">
        Habit 
      </div>
      ...
      {currentDays.map((day) => (
        <div
          key={day}
          className="text-center font-bold text-electric-orange bg-white border border-electric-blue/20"
        >
          <div>{day}</div>
      ...
      {habits.length > 0 ? (
        habits.map((habit) => (
          <React.Fragment key={habit.id}>
            {/* Habit name on left side */}
            <div className="flex items-center gap-2 bg-white sticky left-0 z-0 border border-electric-blue/10 pr-2">
            ...
            {currentDays.map((day) => {
              ...
              return (
                <div
                  key={day}
                  className="flex justify-center items-center border border-electric-blue/10 bg-white"
                >
                  <Checkbox
                    checked={isCompleted}
        ...
        <div className="col-span-full text-center py-6 text-electric-blue">
